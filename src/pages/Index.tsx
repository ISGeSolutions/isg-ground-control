...
import { BarChart3, AlertTriangle, CheckCircle2, Clock, CalendarCheck, LayoutGrid, Calendar, List, Layers, Download, Grid3X3 } from 'lucide-react';
      ...
      <div className="border-b border-border px-4 py-2 flex items-center justify-between bg-card">
        <div className="flex items-center gap-3 text-xs">
          <div className="flex items-center bg-secondary rounded-md p-0.5">
            <button onClick={() => setView('grid')} title="Grid" className={`px-2 py-1 rounded text-[10px] font-semibold transition-colors ${view === 'grid' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
              <LayoutGrid className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => setView('calendar')} title="Calendar" className={`px-2 py-1 rounded text-[10px] font-semibold transition-colors ${view === 'calendar' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
              <Calendar className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => setView('next')} title="Next Departures" className={`px-2 py-1 rounded text-[10px] font-semibold transition-colors ${view === 'next' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
              <List className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => setView('series')} title="Series" className={`px-2 py-1 rounded text-[10px] font-semibold transition-colors ${view === 'series' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
              <Layers className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => setView('heatmap')} title="Heatmap (5)" className={`px-2 py-1 rounded text-[10px] font-semibold transition-colors ${view === 'heatmap' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
              <Grid3X3 className="w-3.5 h-3.5" />
            </button>
          </div>
          <button onClick={() => exportDeparturesToCSV(filtered)} title="Export CSV" className="px-2 py-1 rounded text-[10px] font-semibold text-muted-foreground hover:text-foreground transition-colors">
            <Download className="w-3.5 h-3.5" />
          </button>
          <span className="text-muted-foreground">{filtered.length} departures</span>
        </div>
      </div>
